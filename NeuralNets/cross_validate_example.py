...
def create_input(num_samples, num_features):
...
def create_model(num_features, x, y, num_epochs=10, num_layers=3, num_units=100, learn_rate=0.01):
    # train
    model = Sequential()
...
def predict(model, num_samples, num_features):
    test_x, answ = create_input(num_samples, num_features)
...
for i in num_epoch:    
    x, y = create_input(max_samples, num_features)
    model, h1 = create_model(num_features, x, y, i)
    histories.append(h1)
...
for i in num_samples:    
    x, y = create_input(i, num_features)
    model, h1 = create_model(num_features, x, y, 10)
    histories.append(h1)
...
for i in num_units:    
    x, y = create_input(max_samples, num_features)
    model, h1 = create_model(num_features, x, y, 10, 3, i)
    histories.append(h1)
    test_x, test_y = predict(model, max_samples, num_features)
    plt.subplot(5, 3, fig_num)
    plt.plot(x, y, 'yo')
...
for i in num_layers:    
    x, y = create_input(max_samples, num_features)
    model, h1 = create_model(num_features, x, y, 10, i)
    histories.append(h1)
    test_x, test_y = predict(model, max_samples, num_features)
    plt.subplot(5, 3, fig_num)
    plt.plot(x, y, 'yo')
...
for i in lr_varies:    
    x, y = create_input(max_samples, num_features)
    model, h1 = create_model(num_features, x, y, 10, 3, 100, i)
    histories.append(h1)
    test_x, test_y = predict(model, max_samples, num_features)
